...
func (p *MockParser) ParseProgram() *domain.Program {
	...
	for {
		...
		case domain.INT, domain.DOUBLE, domain.STRING:
...
func (p *MockParser) parseFunction() *domain.FunctionDecl {
	...
	if nameToken.Type != domain.IDENTIFIER {
...
func (p *MockParser) parseGlobalVariable(typeToken domain.Token) *domain.VarDeclStmt {
	nameToken := p.lexer.NextToken()
	if nameToken.Type != domain.IDENTIFIER {
...
func TestParseSimpleProgram(t *testing.T) {
	...
	if mainFunc.ReturnType.String() != "int" {
		t.Errorf("Expected return type 'int', got '%s'", mainFunc.ReturnType.String())
	}
}
...
func TestParseVariableDeclarations(t *testing.T) {
	...
	if len(program.Declarations) > 0 {
		...
		if !ok {
			...
			if varDecl.Type.String() != "int" {
				t.Errorf("Expected type 'int', got '%s'", varDecl.Type.String())
			}
		}
	}
}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			for {
				...
				switch token.Type {
				case domain.FOR, domain.WHILE, domain.IF:
					foundKeywords[token.Literal] = true
				}
			}
...
func TestParseComplexProgram(t *testing.T) {
	...
	expectedMinDecls := 3
	if len(program.Declarations) < expectedMinDecls {
		t.Errorf("Expected at least %d declarations, got %d", expectedMinDecls, len(program.Declarations))
	}
...
func TestParseOperatorPrecedence(t *testing.T) {
	...
	expectedOperators := []domain.TokenType{
		domain.PLUS,     // +
		domain.MULTIPLY, // *
		domain.MINUS,    // -
		domain.DIVIDE,   // /
