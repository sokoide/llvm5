...
import (
	...
	"github.com/sokoide/llvm5/staticlang/codegen"
...
func TestCodeGenBasicProgram(t *testing.T) {
	...
	mainFunc.Body.Statements[0].(*domain.ReturnStmt).Expression.(*domain.LiteralExpr).SetType(domain.NewIntType())
...
func TestCodeGenVariableDeclaration(t *testing.T) {
	...
	// Set types
	mainFunc.Body.Statements[1].(*domain.ReturnStmt).Expression.(*domain.LiteralExpr).SetType(domain.NewIntType())
...
func TestCodeGenFunctionCall(t *testing.T) {
	...
	callExpr := &domain.CallExpr{
		Function:  "print",
		Arguments: []domain.Expression{arg},
	}
...
func TestCodeGenControlFlow(t *testing.T) {
	...
	thenStmt.Expression.(*domain.LiteralExpr).SetType(domain.NewIntType())
	...
	elseStmt.Expression.(*domain.LiteralExpr).SetType(domain.NewIntType())
