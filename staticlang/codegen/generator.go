...
func (g *Generator) generateGlobalVariable(varDecl *domain.VarDeclStmt) error {
	llvmType := g.getLLVMType(varDecl.Type_)
...
func (g *Generator) VisitVarDeclStmt(node *domain.VarDeclStmt) error {
	llvmType := g.getLLVMType(node.Type)
	align := g.getTypeAlign(node.Type)
...
func (g *Generator) VisitAssignStmt(node *domain.AssignStmt) error {
	...
	if err := node.Expression.Accept(g); err != nil {
		return err
	}

	// Store the result
	varType := g.getLLVMType(node.Variable.GetType())
	align := g.getTypeAlign(node.Variable.GetType())

	if ident, ok := node.Variable.(*domain.IdentifierExpr); ok {
...
func (g *Generator) VisitForStmt(node *domain.ForStmt) error {
	...
	if node.Increment != nil {
		if err := node.Increment.Accept(g); err != nil {
			return err
		}
	}
	g.emit("br label %%%s", condLabel)
...
func (g *Generator) VisitReturnStmt(node *domain.ReturnStmt) error {
	if node.Expression != nil {
		...
		returnType := g.getLLVMType(node.Expression.GetType())
...
func (g *Generator) VisitCallExpr(node *domain.CallExpr) error {
	...
	if node.Function == "print" && len(node.Arguments) == 1 {
		if err := node.Arguments[0].Accept(g); err != nil {
			return err
		}

		argType := node.Arguments[0].GetType().String()
		switch argType {
		case "int":
	...
	if returnType == "void" {
		g.emit("call void @%s(%s)", node.Function, args)
	} else {
		g.emit("%%temp_result = call %s @%s(%s)", returnType, node.Function, args)
	}
...
func (g *Generator) VisitLiteralExpr(node *domain.LiteralExpr) error {
	...
	case "string":
		// String literals need special handling
		strValue := strings.Trim(node.Value, "\"")
		length := len(strValue) + 1
