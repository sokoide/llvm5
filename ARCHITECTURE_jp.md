...
- ソース位置追跡を処理
- **トークン分類**: 型キーワード（`int`、`string` など）は特別なトークンではなく識別子として扱われ、型システムによって解決されます
...
### テストアーキテクチャ
テスト戦略は Clean Architecture の原則を維持しています：
- **モックコンポーネント**: 各レイヤーの分離テストを可能にする
- **インターフェース契約**: インターフェース準拠の検証テスト
- **コンポーネント統合**: 適切なコンポーネント間相互作用のエンドツーエンドテスト
- **エラーハンドリング**: 包括的なエラーシナリオのカバレッジ
