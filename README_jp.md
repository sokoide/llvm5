...
Go を使用して構築された、StaticLang プログラミング言語のためのコンパイラプロジェクトです。Clean Architecture の原則に従い、包括的なエラーハンドリングを実装しています。**現在開発中でモックコンポーネントを使用 - まだ本番環境対応ではありません。**

## ⚠️ 開発状況

**重要**: このプロジェクトは現在アクティブに開発中です。コンパイラはデフォルトでモック実装を使用し、まだ実際の LLVM コード生成を行いません。すべてのコンパイルは現在、開発・テスト目的でモック結果を出力します。
...
- **メモリ管理**: 効率的なメモリプールと追跡
- **モック LLVM バックエンド**: 開発・テスト用の完全なモック実装
- **拡張可能な設計**: 将来の拡張のためのプラグイン対応アーキテクチャ

## 🚧 現在の制限事項

- **モックコンポーネント**: すべてのコアコンポーネント（レクサー、パーサー、意味解析器、コードジェネレーター）はデフォルトでモック実装を使用
- **実際の LLVM なし**: コード生成は実際の LLVM IR ではなくモック出力を生成
- **開発専用**: 本番環境での使用には適さない
- **限定的な言語サポート**: 基本的な言語機能を実装、高度な機能は保留中
...
```bash
# 単一ファイルをコンパイル（デフォルトでモックコンポーネントを使用）
./build/staticlang -i hello.sl -o hello.ll -v

# 明示的にモックフラグ付きでコンパイル（開発時に推奨）
./build/staticlang -i hello.sl -o hello.ll -mock -v

# 複数ファイルをコンパイル（すべてモックコンポーネントを使用）
...
- **コードジェネレーター**: 最適化付き LLVM IR 生成（現在はモック）
...
- **現在の状況**: モック実装のみ - まだ実際の LLVM 統合なし
- **アーキテクチャ**: インターフェースを通じて LLVM 機能を抽象化（実際の実装準備済み）
- **モックバックエンド**: 開発・テスト用の完全なモック実装
- **将来の計画**: 将来のバージョンで実際の LLVM 統合を予定
- **最適化**: 設定可能な最適化レベル (0-3) - 現在はシミュレーション
